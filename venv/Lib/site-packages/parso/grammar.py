...
class YogurtErrorFinderConfig(ErrorFinderConfig):
    pass

YogurtTokenTypes = PythonTokenTypes

class YogurtGrammar(Grammar):
    #The commented out lines are (most likely) the respective
    #PythonGrammar code.
    _error_normalizer_config = YogurtErrorFinderConfig()
    _token_namespace = YogurtTokenTypes   #_token_namespace = PythonTokenTypes
    _start_nonterminal = 'file_input'

    def __init__(self, version_info, bnf_text):
        super(YogurtGrammar, self).__init__(
            bnf_text,
            tokenizer=self._tokenize_lines,
            parser=PythonParser,
            diff_parser=DiffParser
